...
function handleAddCardFormSubmit(evt) {
  ...
  // тут надо будет поменять надпись на кнопке...

  // сохраняем данные на сервере...
  Promise.all([saveNewCard(newCardName.value, newCardImageURL.value)]).then(
    ([card]) => {
      // и тут опять поменять надпись на кнопке...
...
function handleEditProfileFormSubmit(evt) {
  ...
  // сохраняем данные на сервере...
  Promise.all([updateUserInfo(userData)]).then(([user]) => {
    // и тут опять поменять надпись на кнопке...
...
function handleEditAvatarFormSubmit(evt) {
  ...
  // записываем значения из поля формы в профиль пользователя
  userData.avatar = avatarUrlInput.value;
